...
4. Billing documents: paid plans produce a **cuenta de cobro** (PDF, `PlanInvoice` with status `ISSUED`) attached to the 7-day and 1-day reminders, and a **factura / recibo** (`PAID`) attached to the activation email when the operator records the payment. Numbering is `MS-YYYY-NNNN`; the issuer block comes from `INVOICE_ISSUER_NAME` / `INVOICE_ISSUER_DETAILS` (`|`-separated lines). PDFs are generated without external libraries (`src/server/utils/simple-pdf.ts`). Customers download them from **Mi plan**; the operator manages all of them in **Admin → Facturas** (register a payment, which activates or renews the plan for the invoiced period and emails the factura; void; resend; download) and can issue a cuenta de cobro ahead of a sale from **Admin → Clientes → Cuenta de cobro**. A renewal of the same plan starts when the current period ends, so paying early never shortens it.
